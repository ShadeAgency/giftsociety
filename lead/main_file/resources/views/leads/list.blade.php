                                        ...
                                        @foreach ($leads as $lead)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a @if($user->avatar) href="{{asset('/storage/avatars/'.$user->avatar)}}" @else href="{{asset('/storage/avatars/avatar.png')}}" @endif class="btn btn-sm mr-1 p-0 rounded-circle" target="_blank">
                                                            <img alt="image" data-toggle="tooltip" data-original-title="{{$user->name}}" @if($user->avatar) src="{{asset('/storage/avatars/'.$user->avatar)}}" @else src="{{asset('custom/img/avatar/avatar-1.png')}}" @endif class="rounded-circle " width="25" height="25">
                                                        </a>
                                                    @endforeach
                                                </td>
                                                    ...
                                                    <td class="Action">
                                                        ...
                                                        @if($lead->is_active)
                                                                    <div class="action-btn bg-warning ms-2">
                                                                        <a href="{{route('leads.show',$lead->id)}}" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('View Lead')}}"><i class="ti ti-eye text-white"></i></a>
                                                                    </div>
                                                                @endif
                                                            @endcan
                                                            @can('Edit Lead')
                                                                <div class="action-btn btn-info ms-2">
                                                                    <a href="#" data-size="lg" data-url="{{ URL::to('leads/'.$lead->id.'/edit') }}" data-ajax-popup="true"  class="mx-3 btn btn-sm d-inline-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('Edit Lead')}}" ><i class="ti ti-pencil text-white"></i></a>
                                                                </div>
                                                            @endcan
                                                            @can('Delete Lead')
