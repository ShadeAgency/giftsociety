                                                ...
                                                @if(count($pipelines) > 1)
                                                    @can('Delete Pipeline')
                                                        <div class="action-btn bg-danger ms-2">
                                                            <form method="POST" action="{{ route('pipelines.destroy', $pipeline->id) }}">
                                                                    @csrf
                                                                    <input name="_method" type="hidden" value="DELETE">
                                                                    <button type="submit" class="mx-3 btn btn-sm d-inline-flex align-items-center show_confirm" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('Delete')}}">
