        ...
        @foreach($users as $user)
            <div class="col-md-3 col-sm-6 col-md-3">
                <div class="card text-white text-center">
                        ...
                        <div class="card-header border-0 pb-0">
                            @if(\Auth::user()->type == 'Super Admin')
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="d-grid">
                                        <button class="badge bg-primary p-2 px-3 rounded">{{!empty($user->getPlan()->first())?$user->getPlan()->name:''}}
                                        </button>
                                    </div>
                                </div>
                            @else
                                            ...
                                            @can('Delete User')
                                                {!! Form::open(['method' => 'DELETE', 'route' => ['users.destroy', $user['id']],'id'=>'delete-form-'.$user['id']]) !!}
                                                    <a href="#!" class="dropdown-item show_confirm">
                                                       <i class="ti ti-archive ms-1"></i><span class="ms-1">@if($user->delete_status == 0){{__('Delete')}} @else {{__('Restore')}}@endif</span></a>
                                                {!! Form::close() !!}
                                            @endcan


                                             <a href="#" class="dropdown-item" data-url="{{route('user.reset',\Crypt::encrypt($user->id))}}" data-ajax-popup="true" data-size="md" data-title="{{__('Reset Password')}}" data-toggle="tooltip" data-original-title="{{__('Reset Password')}}"><i class="ti ti-key mx-1"></i>
                                                <span>{{__('Reset Password')}}</span>
                                            </a>

                                            @if(\Auth::user()->type == 'Super Admin')
                                                <a href="#" class="dropdown-item" data-size="lg" data-url="{{ route('plan.upgrade',$user->id) }}" data-ajax-popup="true" data-title="{{__('Upgrade Plan')}}" data-toggle="tooltip" data-original-title="{{__('Upgrade Plan')}}"><i class="ti ti-award mx-1"></i>
                                                    {{__('Upgrade Plan')}}
                                                </a>
                                            @endif
                    ...
                    <div class="card-body">
                        {{-- <img @if($user->avatar) src="{{asset('/storage/avatars/'.$user->avatar)}}" @else src="{{asset('custom/img/avatar/avatar-1.png')}}" @endif alt="user-image" class="img-fluid rounded-circle" style="height100px;width:100px;"> --}}
                        <a href="{{(!empty($user->avatar))? asset(Storage::url("avatars/".$user->avatar)): asset(Storage::url("avatars/avatar.png"))}}" target="_blank">
