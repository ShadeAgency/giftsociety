                        ...
                        <table class="table pc-dt-simple">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($invoice->status == 0)
                                            <span class="badge badge-pill p-2 rounded bg-primary">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 1)
                                            <span class="badge badge-pill p-2 rounded bg-danger">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 2)
                                            <span class="badge badge-pill p-2 rounded bg-warning">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 3)
                                            <span class="badge badge-pill p-2 rounded bg-success">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 4)
                                            <span class="badge badge-pill p-2 rounded bg-info">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @endif
                                    </td>
                                    @if(Auth::user()->type != 'Client')
                                        <td class="text-right">
