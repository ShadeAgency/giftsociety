...
class DealController extends Controller
{
    ...
    public function deal_list()
    {
        ...
        if($usr->can('Manage Deals'))
        {
            ...
            $deals       = Deal::where('created_by', '=', $usr->ownerId())->where('pipeline_id', '=', $pipeline->id)->get();
            $curr_month  = Deal::where('created_by', '=', $usr->ownerId())->where('pipeline_id', '=', $pipeline->id)->whereMonth('created_at', '=', date('m'))->get();
            $curr_week   = Deal::where('created_by', '=', $usr->ownerId())->where('pipeline_id', '=', $pipeline->id)->whereBetween(
                'created_at', [
                                \Carbon\Carbon::now()->startOfWeek(),
                                \Carbon\Carbon::now()->endOfWeek(),
                            ]
            )->get();
            $last_30days = Deal::where('created_by', '=', $usr->ownerId())->where('pipeline_id', '=', $pipeline->id)->whereDate('created_at', '>', \Carbon\Carbon::now()->subDays(30))->get();
